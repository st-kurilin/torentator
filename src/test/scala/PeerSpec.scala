...
class PeerPoorSpec extends FlatSpec with Matchers {
  ...
  it should "deserialize to None if message is uncorrect" in {
    ...
    Seq(
      ...
      "not enough data to parse Have" -> Seq(0, 0, 0, 4, 4, 0, 0, 1),
      "not enough data to parse Request" -> Seq(0, 0, 0, 12, 6, 0, 0, 1, 2, 0, 0, 1, 3, 1, 2, 3),
      "to big data for Request" -> Seq(0, 0, 0, 14, 6, 0, 0, 1, 2, 0, 0, 1, 3, 1, 2, 3, 1, 1),
      "not enough data to parse Piece" -> Seq(0, 0, 0, 8, 7, 0, 0, 1, 2, 0, 0, 1),
      "not enough data to parse Cancel" -> Seq(0, 0, 0, 12, 8, 0, 0, 1, 2, 0, 0, 1, 3, 1, 2, 3),
...
class PeerActorSpec extends torentator.ActorSpec("PeerSpec") {
  ...
  "Peer" must {
    "sends hanshake just after creation" in {
      val expectedHandshake = Peer.handshakeMessage(trackerId, infoHash)
